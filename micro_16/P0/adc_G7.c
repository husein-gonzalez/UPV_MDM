...
//CONFIGURAMOS EL ADC1 PARA MUESTREAR AN4,AN5,AN8 y AN9 por canal CH0
...
DMA6CONbits.DIR = 0; // 1=lee de RAM y escribe a perif?rico
...
DMA6CONbits.MODE = 0; 	// 11:One PP / 10:Continuo + PP
						// 01: One / 00: Continuo
...
IFS4bits.DMA6IF = 0; //Borra flag Int.
IEC4bits.DMA6IE = 1; //permite interrupciones DMA
DMA6CONbits.CHEN = 1; // Activa canal 2 DMA
...
//================================================================
//========== CAD 2 DMA Scatter/gather MODE ============
//inicializa CAD2 para DMA 
//CONFIGURAMOS EL ADC2 PARA MUESTREAR AN4,AN5,AN8 y AN9 por canal CH0
void Inic_CAD_2_DMA(void) 
{
AD2CON1= 0; // 0x00E0
//_ADON= 0; // CAD OFF
//_ADSIDL = 0; //1=Se para en modo Idle / 0=Continua en modo Idle
//_ADDMABM = 0; //1=bufer DMA en orden de conversi?n***
// 0=bufer DMA en modo Scatter/Gather
//_AD22B = 0; //1=Modo operacion de 12 bits / 0=Modo operacion de 10 bits
//_FORM = 0; //Formato datos salida: 00=Integer (0000 00dd dddd dddd)
// 01=Signed integer (ssss sssd dddd dddd)
AD2CON1bits.SSRC = 0b111; //Termina muestreo y comienca conversion del CAD
//111=Auto-coonvert
// 010=TMR3 ADC1 y TMR5 ADC2
// 001=INT0 / 000= hay que borrar SAMP
//_SIMSAM = 0; //1=Muestrea simultaneamente
// 0=Muestrea canales individuales sequencialmente
AD2CON1bits.ASAM = 1; //1=Muestreo comienza seguido de la conversion
// 0=comienza cuando SAMP=1
//_SAMP
//_DONE
AD2CON2= 0; // 0x00E0
//_VCFG = 0; //Vref+- 000=ACdd y AVss / 001=Vref+ y AVss
// 010=AVdd y Vref- / 011=Vref+ y Vref-
AD2CON2bits.CSCNA = 1; //1=Escaneo canal cero / 0= no escaneo
//_CHPS = 0; //Canales utilizados:00=CH0 /01=CH0 y CH1 /1x= CH0,CH1,CH2 y CH3
//_BUFS es solo de lectura: con BUFM=1-> 1=activa 2? mitad de buffer
// 0= activa 1? mitad
AD2CON2bits.SMPI = 3; //Incrementos dir DMA +1 o
//_SMPI = 0; //Incrementos dir DMA +1 o
// sin DMA genera una interrupci?n cada cada SMPI+1
//_BUFM = 0; //1= 2 medios Buffers de 8 / 0= 1 buffer de 16
//_ALTS = 0; //Modo de muestreo: 1= Alternado / 0= Siempre canal A
AD2CON3 = 0;
//_ADRC = 0; //1= reloj RC / 0= Reloj CPU
AD2CON3bits.SAMC = 31; //Tiempo auto muestreo0 n?de Tad (de 0 a 31Tad)
AD2CON3bits.ADCS = 3; //Tad= >75ns = Tcy(ADCS+1) -> a 80MHz Tcy= 25ns
// Tad = (3+1)Tcy
AD2CON4= 0;
//_DMABL = --; //Cada buffer contiene 8 palabras
AD2CHS123= 0; // reg. seleccion de entrada canal 1,2 y 3
//_CH123NA= 0; //Selecci?n entrada negativa Canales 1, 2, y 3 pata Sample A
//0x= CH1=CH2=CH3-> Vref-
// 10= CH1=AN6,CH2=AN7,CH3=AN8
// 11= CH1=AN9,CH2=AN10,CH3=AN11
...
//_CH123NB = 0;
//_CH123SB = 0;
AD2CHS0= 0; // Seleccion de entrada canal 0
//_CH0NA = 0; //Entrada negativa 1= AN1 / 0 = Vref-
//_CH0SA = 5; //Entrada positiva = canal(CH0SA)
//_CH0SA = 4;
//_CH0SB = 3;
...
AD2CON4bits.DMABL = 2; 	//N? datos por entrada (0->1,1->2,2->4,3->8,...,7->128 
						...
						AD2CON1bits.ADON = 1; //Habilita convertidor
//_SAMP= 1; //Lanza conversion
//------------------------------------------------------------------
DMA5CON = 0;
// CHEN CANAL habilitado / deshabilitado
...
DMA5CONbits.DIR = 0; // 1=lee de RAM y escribe a perif?rico
// 0=lee de perif?rico y escribe en RAM
// HALF interrupcion a mitad / al final
// NULLW Escribe nulos / operacion normal
DMA5CONbits.AMODE = 2;    // 00 Indir.Reg con Post-Incr
                        //10: Direc.Indirect,Perif / 01 Indir.Reg sin Post-incr
                     
DMA5CONbits.MODE = 0; 	// 11:One PP / 10:Continuo + PP
						// 01: One / 00: Continuo
 
 
// DMA0REQ
DMA5REQbits.FORCE = 0; // manual / autom?tico
// IRQSEL
// 0x00 INT0
// 0x01 IC1
// 0x02 OC1
...
// 0x0B UART1RX
// 0x0C UART1TX
DMA5REQbits.IRQSEL = 0x15; // ADC1 //DMA_IRQ_ADC1 //FIXME usar el ADC2
// 0x15 ADC2
// 0x1E UART2RX
// DMA2REQbits.IRQSEL = 0x1F; // UART2TX
// 0x21 SPI2
...
IFS3bits.DMA5IF = 0; //Borra flag Int.
IEC3bits.DMA5IE = 1; //permite interrupciones DMA
DMA5CONbits.CHEN = 1; // Activa canal 2 DMA
